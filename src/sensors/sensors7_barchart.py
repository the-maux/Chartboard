...
def sonde7(tester=None, tile_id='barjs_ex', isHorizontal=False):
    ...
    meta = updateMeta()
    CHARTBOARDAnswer = sendUpdateByApi(data=data, tileTemplate=TILE_TEMPLATE, tileId=tile_id, tester=tester, meta=meta)
    end(title=f'sensors7 -> {tile_id}', startTime=start_time, CHARTBOARDAnswer=CHARTBOARDAnswer, tileId=tile_id)

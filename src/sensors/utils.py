...
from src.Chartboard.app.properties import CHARTBOARD_URL, COLOR_TAB, LOG


def end(title=None, startTime=None, CHARTBOARDAnswer=None, tileId=None):
    """ Eazy way to end sensors, print the action time & http answer of Chartboard """
    if LOG:
        if CHARTBOARDAnswer.status_code != 200:
            print(f'[ERROR] POST tile:{tileId} Chartboard/push => ({CHARTBOARDAnswer.status_code}): ', flush=True)
        else:
